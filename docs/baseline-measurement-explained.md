...
이 문서는 `docs/measurements/baseline-latest.md` 형식의 baseline 리포트를 해석하기 위한 안내서입니다.
...
요약하면, "MockMvc 기준 서버 내부 요청 처리 시간이 얼마나 걸리는가"와 "그 과정에서 `prepareStatementCount`가 어떻게 집계되는가"를 함께 기록합니다.
...
각 요청 처리 시간 섹션에는 다음 항목이 공통으로 포함됩니다.

- `sampleCount`
  실제로 측정한 요청 수입니다.
- `minMs`
  가장 빨랐던 한 번의 요청 처리 시간입니다.
- `avgMs`
  전체 평균 요청 처리 시간입니다.
- `p95Ms`
  느린 요청 구간까지 반영한 대표 지표입니다.
- `maxMs`
  가장 느렸던 한 번의 요청 처리 시간입니다.

서버 처리 특성을 볼 때는 보통 `avgMs`보다 `p95Ms`를 더 중요하게 봅니다.

- 평균값은 일부 매우 빠른 요청의 영향을 크게 받을 수 있습니다.
- 반면 `p95Ms`는 느린 요청 구간의 서버 처리 편차를 더 잘 반영합니다.

예를 들어 room list의 `avgMs`가 1100ms이고 `p95Ms`가 1450ms라면, MockMvc 기준 서버 내부 요청 처리 시간은 평균적으로 1.1초 수준이고 느린 구간까지 보면 약 1.45초까지 걸릴 수 있다고 해석할 수 있습니다.

## Stability 항목 해석

각 요청 처리 시간 섹션의 마지막에는 `stability` 정보가 포함됩니다.
...
`Room List Query Count`는 요청 처리 시간 대신 Hibernate `prepareStatementCount` 기준 query count를 보여 주는 섹션입니다.

- `counts`
  10회 반복 측정한 `prepareStatementCount` 목록입니다.
- `maxDeviation`
  반복 측정 사이의 최대 편차입니다.
- `note`
  어떤 지점에서 어떤 방식으로 측정했는지 설명하는 메모입니다.

예를 들어 `counts`가 `[161, 161, 161, ...]`이라면, 채팅방 목록 조회 한 번마다 Hibernate `prepareStatementCount`가 항상 161로 집계됐다는 뜻입니다.

이 값은 실제 DB 실행 횟수와 완전히 같은 값이라고 단정할 수는 없지만, N+1 문제나 과도한 조회 같은 구조적 비효율을 비교할 때 중요한 지표입니다.
...
현재 baseline 구조에서는 room list 지표가 특히 중요합니다. 현재 구현은 페이지 크기만큼 잘라내기 전에 전체 활성 방을 먼저 계산하므로, page size보다 활성 채팅방 총수의 영향을 더 크게 받습니다. 이 때문에 room list 요청 처리 시간과 query count가 함께 증가할 수 있습니다.
