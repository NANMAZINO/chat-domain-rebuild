...
이후 Redis 방향은 `휴면 room summary cache + Pub/Sub`으로 정리한다. Redis를 DTO cache 전반에 넓게 적용하지 않고, 실시간 분산 이벤트 전파를 중심 역할로 둔다. room summary cache는 예외적으로 유지하되, 사용자별 상태가 없는 휴면 방에만 제한한다.
